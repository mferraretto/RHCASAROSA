...
const labels = {
  ...
  'employee.paystub': (item) => `📄 Holerite gerado para ${item.meta?.name || item.meta?.id || 'colaborador'} (${item.meta?.reference || 'referência'})`,
  'employee.export.single': (item) => `📤 Exportação individual: ${item.meta?.name || item.meta?.id || 'colaborador'}`,
  'employee.export.list': (item) => `📑 Exportação de colaboradores (${item.meta?.total || 0} registros)`,
